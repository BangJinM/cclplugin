...
export class GraphAssetMgr {
    ...
    public async open(): Promise<boolean> {
        try {
            const result = await Editor.Dialog.select({
                title: Editor.I18n.t('bt-graph.messages.titles.open'),
                path: PROJECT_PATH,
                type: 'file',
                multi: false,
    ...
    public async createNew(type: string): Promise<boolean> {
        try {
            const result = await Editor.Dialog.save({
                title: Editor.I18n.t('bt-graph.messages.save.title'),
                path: join(Editor.Project.path, 'assets', 'New BT Graph'),
                filters: [{
                    name: 'New BT Graph',
                    extensions: ['btgraph'],
                }],
    ...
    public async checkIfSave(): Promise<boolean> {
        if (this.shaderGraphAssetInfo && !existsSync(this.shaderGraphAssetInfo.file)) {
            const result = await Editor.Dialog.warn(Editor.I18n.t('bt-graph.messages.missing_assets.detail'), {
                title: Editor.I18n.t('bt-graph.messages.titles.normal'),
                default: 0,
                cancel: 1,
                buttons: [
                    Editor.I18n.t('bt-graph.buttons.save'),
                    Editor.I18n.t('bt-graph.buttons.unsaved'),
                ],
            });
            ...
            const result = await Editor.Dialog.warn(Editor.I18n.t('bt-graph.messages.save.detail'), {
                title: Editor.I18n.t('bt-graph.messages.titles.normal'),
                default: 0,
                cancel: 1,
                buttons: [
