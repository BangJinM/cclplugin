...
export const DefaultConfig: FloatWindowConfig = {
    key: 'create-node',
    tab: {
        name: 'i18n:bt-graph.create_node.menu_name',
        show: false,
    },
    dontSave: true,
    base: {
        title: 'i18n:bt-graph.create_node.title',
        width: '380px',
        height: '250px',
        minWidth: '200px',
        minHeight: '200px',
...
export const component = defineComponent({
    ...
    setup(props, ctx) {
        ...
        const onShowCreateNodeWindow = (id: string) => {
            parentId = id
        ...
        function createNode(addOptions: GraphEditorAddOptions) {
            ...
            let node = GraphNodeMgr.Instance.createNode(addOptions)
            GraphNodeMgr.Instance.addNode(node, parentId)
            onClose();
        }
    ...
    template: commonTemplate({
        css: 'create-node',
        header: `
<ui-label class="title-label" value="i18n:bt-graph.create_node.title"></ui-label>
<ui-button class="close" transparent
  tooltip="i18n:bt-graph.create_node.close.tooltip"
  @click="onClose"
>
...
<div class="search-group">
  ...
  <ui-input ref="searchInputRef" class="input"
    :value="searchValue"
    placeholder="i18n:bt-graph.create_node.search_input.placeholder"
